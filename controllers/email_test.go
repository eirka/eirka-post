...
func TestEmailController(t *testing.T) {
	...
	assert.Equal(t, second.Body.String(), `{"success_message":"Email Updated"}\n`, "HTTP response should match")
	...
	assert.Equal(t, third.Body.String(), `{"error_message":"Email address the same"}\n`, "HTTP response should match")
	...
	assert.Equal(t, fourth.Body.String(), `{"success_message":"Email Updated"}\n`, "HTTP response should match")
